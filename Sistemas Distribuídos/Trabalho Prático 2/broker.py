...
def shered_resource_management():
    ...
    while True:
        ...
        if len(Requests) >= 1:
            ...
            continue
        lock.release()
        msg = f'{index} => <{username}> esta usando \nFila de espera para usar: {str([user for i, cli, user in Requests])}'
        print(msg)
        ...
        try:
            ...
            if Type == Types.ACQUARE:
                ...
                if client not in Address:
                    ...
                    lock.release()
                    flashmsg = f'O cliente <{username}> se inscreveu'
                    print(flashmsg)
