# -*- coding: utf-8 -*-
"""Lista8_PDI_Last.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1poFQ-xC2L_Eq70YTX2lE4Q_J54z6MXrT
...
def feature_extraction(m_path, img_files):
  ...
  for l, file in enumerate(img_files):
      ...
      for j in range(len(angs)):
          n1img = transform.rotate(img, angs[j])
          for k in range(len(tams)):
            n2img = transform.rescale(n1img, tams[k])
            for i in range(len(ruidos)):
              n3img = util.random_noise(n2img, mode='s&p', amount = ruidos[i])
              
              hu_X[pos, :] = Hu_moments(n3img)
              hu_Y[pos] = l
              pos += 1
...
x_train, x_test, y_train, y_test = train_test_split(hu_Xn, hu_Y)
